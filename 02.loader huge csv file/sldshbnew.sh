...
if [ $# != 3 ]
then
   echo "Usage: sld username password table_name"
else
  
#  export NLS_CALENDAR='GREGORAIN'
export NLS_DATE_FORMAT='dd/mm/yyyy'
echo "NLS Calendar SET TO : "$NLS_CALENDAR
echo "NLS DATE FORMAT TO : "$NLS_DATE_FORMAT

for FILE_NAME in /Source2/shb/*.unls
do
   ...
   split -C $PIECE_SIZE"m" $FILE_NAME $LOAD_DIR/data_
   ...
   from user_tab_columns where table_name = upper('$TABLE_NAME') order by column_id;
   spool off
   exit;
EOF
   ...
   #   echo "CONTINUE_LOAD DATA" >> $LOAD_DIR/ctl.tmpl
   ...
   # echo "FIELDS TERMINATED BY ',' optionally enclosed by '\"' " >> $LOAD_DIR/ctl.tmpl
